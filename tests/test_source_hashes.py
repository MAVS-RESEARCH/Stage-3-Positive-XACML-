...
def test_phase5_corruption_artifacts_sealed():
    """C09 extension: Phase-5 corruption artifacts prove 5/5 detection."""
    # [P5-LOG-T70] Test step: assert Phase-5 corruption seal.
    print("[P5:test:hashes:070] checking Phase-5 corruption artifacts",
          flush=True)
    audits = os.path.join(REPO_ROOT, "artifacts", "audits")
    if not os.path.isfile(os.path.join(audits, "corruption_pdp.json")):
        import pytest
        pytest.skip("Phase-5 corruption artifacts absent (pre-Phase-5)")
    for name in ("pdp", "request", "response", "policy", "spec"):
        with open(os.path.join(audits, "corruption_%s.json" % name),
                  encoding="utf-8") as handle:
