...
def cmd_build_packet(args):
    ...
    for rel in ("pom.xml", "pdp-engine/pom.xml"):
        copy_into(repo_root, "external/authzforce-repo/" + rel,
                  "candidates/built/poms/" + rel.rsplit("/", 1)[-1])
    jar_rels = [
    ...
    with open(os.path.join(dest, "candidates", "lambda_manifest.json"),
              encoding="utf-8") as handle:
        operative = json.load(handle)["pre_hash"]
    lineage["current"] = {
        "pre_hash": operative,
        "scope": "launch packet manifest (see lambda_manifest.json)",
    }
    lineage.setdefault("history", []).append({
        "pre_hash": "3e8cf8ffba078c54c25867cb3aa2e0c232165bbded136efb2a3f"
                    "58407f4e221a",
        "scope": "rev001 manifest",
        "status": "HISTORICAL (superseded at rev002; value preserved)",
    })
    with open(os.path.join(dest, "candidates", "pre_hash_lineage.json"),
    ...
    for tool_rel in ("src/audit/recompute_lambda.py",
                     "src/audit/verify_deployment_set.py",
                     "src/xacml/build_repaired_requests.py"):
        live = sha256_file(os.path.join(repo_root, *tool_rel.split("/")))
        name = tool_rel.rsplit("/", 1)[-1]
        candidates = {
            "recompute_lambda.py": "candidates/recompute_lambda.py",
            "verify_deployment_set.py":
            "candidates/verify_deployment_set.py",
            "build_repaired_requests.py": "candidates/builder_source.py",
        }
        staged = sha256_file(os.path.join(
            dest, *candidates[name].split("/")))
        if live != staged:
    ...
    manifest_json = json.dumps(
        {"packet_id": "PC-XACML-S3PLUS-v1-launch-packet",
    ...
    try:
        ...
        pass
    record = {
        "launch_id": "PC-XACML-S3PLUS-v1-launch-001",
        "semantic_revision": 4,
        "frozen": True,
